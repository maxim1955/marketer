...
<body>
  ...
  <div id="table-container">
    ...
    <div class="action-buttons-container mb-3 align-items-start">
      ...
      <button class="action-button btn btn-success btn-sm" onclick="openConfirmationModal()">Обновить документ</button>
    ...
    <script>
      ...
      var orderForApplicationIDS = [];
      var initialIds = [];
      var initialIdsNewData = [];
      // var newOrder = [];
      // сделать функцию для отслеживания id как для appId
      ...
      // Функция для обновления массивов порядка при изменении таблицы
      function updateOrderArrays() 
      {
        let newOrder = [];
        let newOrderIDS = [];
        ...
        $('#sortableTable tbody tr').each(function() {
          ...
          newOrderIDS.push(parseInt(idValue));
        });

        if (document.getElementById("moveCheckbox").checked = true) 
        {
          if (!arraysEqual(initialIds, newOrder)) 
          {
            orderUpdateBuffer = newOrder;
            initialIds = newOrder.slice();
            console.log('Order by applicationID:', initialIds);
          }
        } 
        else 
        {
          if (!arraysEqual(initialIdsNewData, newOrderIDS)) 
          {
            orderForApplicationIDS = newOrderIDS;
            initialIdsNewData = newOrderIDS.slice();
            console.log('Order by id:', initialIdsNewData);
            restoreTableOrder(newOrder);
          }
        }
      }

      function restoreTableOrder(newOrder) 
      {
        var tableBody = $('#sortableTable tbody');
        var rows = tableBody.find('tr').toArray();

        rows.sort(function(a, b) {
          var dataIndexA = parseInt($(a).find("td:nth-child(3)").text());
          var dataIndexB = parseInt($(b).find("td:nth-child(3)").text());

          return newOrder.indexOf(dataIndexA) - newOrder.indexOf(dataIndexB);
        });

        tableBody.empty();
        tableBody.append(rows);

        console.log('Table order restored.');
      }
      ...
      // Вызовите функцию для инициализации Sortable
      initializeSortable();
      ...
      function showDataInTable(newData) {
        ...
        newData.forEach(function(item, index) {
          var row = '<tr>' +
            ...
            '<td><input type="checkbox" id="checkbox' + item.applicationId + '"></td>' +
            '<td class="applicationID">' + item.applicationId + '</td>' +
            '<td class="totalTime">' + item.totalTimesInput + '</td>' +
            '<td class="Period">' + item.selectedPeriod + '</td>' +
            '<td class="full_address">' + item.fullAddress + '</td>' +
            '<td class="logisticCommentary">' + item.logisticCommentary + '</td>' +
            '</tr>';
      ...
      function showOnMap(newCoords, newData, polygons) 
      {
        ...
        newCoords.forEach(function(address, index) {
          ...
          if (!isNaN(lat) && !isNaN(lng)) {
            ...
            // Создайте маркер и добавьте его в массив
            var marker = L.marker([lat, lng], {
                icon: customIcon,
                originalOrder: markerCount
              }).addTo(map)
              .bindPopup('<th>' + address.full_address + '</th>' + '<p>Номер: ' + findApplicationIdByAddress(address, newData) + '</p>');
            ...
            markersInfoArray.push({
              order: markerCount,
              applicationId: findApplicationIdByAddress(address, newData),
            });
        ...
        console.log(markersInfoArray);
      }

      function findApplicationIdByAddress(address, newData) 
      {
        var matchingData = newData.find(function(data) {
          return data.fullAddress === address.full_address;
        });

        return matchingData ? matchingData.applicationId : 'N/A';
      }
      ...
      function updatePolyline(newData) {
        ...
        if ($('#moveCheckbox').prop('checked')) {
          ...
          if (selectedMarkers.length >= 2) {
            ...
            var orderAfterPolyline = selectedMarkers.map(function(marker, index) {
              return {
                order: index + 1,
                address: marker.getPopup().getContent(),
                applicationId: newData[marker.options.originalOrder - 1].applicationId,
      ...
      function updateTableData(selectedMarkersOrder) {
        var addresses = selectedMarkersOrder.map(function(markerData) {
          return markerData.address;
        });

        var applicationIds = selectedMarkersOrder.map(function(markerData) {
          return markerData.applicationId;
        });

        console.log(selectedMarkers);

        // Обновление порядковых номеров в маркерах
        selectedMarkersOrder.forEach(function(updatedMarkerData, index) {
          var marker = findMarkerByApplicationId(updatedMarkerData.applicationId);
          if (marker && marker.setIcon) { // Проверка наличия метода setIcon
            updatedMarkerData.order = index + 1;
            // Обновите порядковый номер в маркере на карте
            var markerHtml = '<div class="custom-marker">' + updatedMarkerData.order + '</div>';
      ...
      function findMarkerByApplicationId(applicationId) {
        return markersArray.find(function(marker) {
          return marker.options.applicationId === applicationId;
        });
      }
      ...
      function updateMarkersOrder() {
        console.log('Updating markers order...');
        markersArray.forEach(function(marker, index) {
          ...
          console.log('Marker', marker.options.originalOrder, 'moved to', newIndex);
        });

        console.log('Markers order updated.');
      }
      ...
      function openConfirmationModal() {
        ...
        $('#confirmationDissmis').one('click', function() {
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <div class="modal-footer">
            <button type="button" class="btn btn-secondary" id="confirmationDissmis" data-dismiss="modal">Нет</button>
